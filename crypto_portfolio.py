...
def historical_tradedata_analysis(tradepairs, from_datetime, interval='1h',
    ...
    # - Return bar
    plt.figure(figsize=figsize)
    ax = periodized_ret.plot.bar(ylabel='%', color='goldenrod')
    ...
    # - Volatility bar
    plt.figure(figsize=figsize)
    ax = periodized_vol.plot.bar(ylabel='%', color='salmon')
    ax.yaxis.set_major_formatter(_ticklabel_formatter)
    ...
    # - Sharpe Ratio bar
    plt.figure(figsize=figsize)
    ax = sharpe_ratio.plot.bar(color='green')
...
def portfolio_pie_plot(weights, rows_index, name, figsize=(5, 5)):
    ...
    weights100 = (weights100 > 1e-8) * weights100
